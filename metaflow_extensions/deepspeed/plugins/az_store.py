...
class AzureBlob(object):
    ...
    def __init__(
        ...
        if blob_path is None and run_pathspec is None:
            raise ValueError(
                "Either blob_path or run_pathspec must be specified in the AzureBlob instantiation."
            )
        if blob_path and run_pathspec:
            raise ValueError(
                "Cannot specify both blob_path and run_pathspec in the AzureBlob instantiation."
            )

        # TODO: tmp
        if blob_path:
            raise NotImplementedError(
                "Custom blob_path is not yet implemented in AzureBlob. Pass flow_name and run_id instead."
            )
    ...
    def put_files(self, key_paths: List[Tuple[str, str]], overwrite=False):
        ...
        for key, path in key_paths:
            ...
            results.append(self.get_datastore_key_location(key))
        return results
