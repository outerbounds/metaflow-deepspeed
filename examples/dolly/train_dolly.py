...
class TrainDolly(FlowSpec):
    ...
    deepspeed_config = Parameter(
        name="config", default="ds_config.json", help="DeepSpeed config file."
    )
    # TODO: The deepspeed config file could use IncludeFile. It is simple, but a bit clunky to use Parameter + --package-suffixes.

    # hyperparameters
    learning_rate = Parameter(
        "learning_rate", default="1e-5", help="Learning rate for training."
    )
    batch_size = Parameter(
        "batch_size", default="16", help="Batch size for training & evaluation."
    )

    debug_log = Parameter(
        "debug_log", is_flag=True, default=False, help="Whether to log debug messages."
    )
    sample_percentage = Parameter(
        "sample-percentage",
        default=None,
        help="Percentage of data to use for training.",
    )
    push_model_to_s3 = Parameter(
        "push-model-to-s3",
        default=True,
        help="Whether to push model to S3 after training.",
    )
    ...
    def train(self):
        ...
        current.deepspeed.run(
            ...
            + (["--debug-log"] if self.debug_log else [])
            + (
                ["--sample-percentage", self.sample_percentage]
                if self.sample_percentage
                else []
            ),
        )
        ...
        if self.push_model_to_s3:
            ...
            self.s3_output_dir = os.path.join(
                self.training_output_dir, checkpoint_dir_name
            )
            with S3(run=self) as s3:
                self.filepath_tuples = [
                    (
                        f"{self.s3_output_dir}/{f}",
                        os.path.join(self.local_output_dir, f),
                    )
