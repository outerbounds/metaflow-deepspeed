...
def preprocess_batch(
    batch: Dict[str, List], tokenizer: AutoTokenizer, max_length: int = MAX_LENGTH
) -> dict:
    return tokenizer(
        batch["text"],
...
def load_training_dataset(
    training_data_id: str = DEFAULT_TRAINING_DATASET, split: str = "train"
) -> Dataset:
    ...
    dataset = dataset.filter(
        lambda rec: not rec["text"].strip().endswith("### Response:")
    )
...
def preprocess_dataset(
    tokenizer: AutoTokenizer, max_length: int = MAX_LENGTH, seed=DEFAULT_SEED
) -> Dataset:
...
def train(
    ...
    split_dataset = processed_dataset.train_test_split(
        test_size=test_size, seed=seed, train_size=sample_percentage
    )
...
@click.option(
    "--bf16", type=bool, default=True, help="Whether to use bf16 (preferred on A100's)."
)
@click.option(
    "--debug-log", type=bool, default=False, help="Whether to log debug messages."
)
@click.option(
    "--sample-percentage",
    type=float,
